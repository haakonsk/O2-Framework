...
sub new {
  ...
  $obj->_debug('CALLED BY[' . join ('|', caller) . "] ->new DBH: " . join ',', %login);
  return $obj;
}
#-----------------------------------------------------------------------------
...
sub getDbh {
  ...
  $obj->_debug("DBI connect: ('$login[0]', '$login[1]', ...)");
...
sub selectColumn {
  ...
  $obj->_debug('selectColumn(' . $obj->_expandPH($statement, @params) . ')', 2);
...
sub fetchAll {
  ...
  $obj->_debug('fetchAll(' . $obj->_expandPH($statement, @placeHolders) . ')', 2);
...
sub selectHash {
  ...
  $obj->_debug('selectHash(' . $obj->_expandPH($sql, @placeholders) . ')', 2);
...
sub clone {
  my ($obj) = @_;
  my %login = %{ $obj->{login} };
  $obj->_debug('clone() CALLED BY[' . join ('|', caller) . "] ->new DBH: " . join ',', %login);
...
sub insert {
  ...
  $obj->_debug('insert(' . $obj->_expandPH($sql, @values) . ')');
...
sub idUpdate {
  ...
  $obj->_debug('idUpdate(' . $obj->_expandPH($sql, @values, $idValue) . ')');
  my @encodedValues = $obj->_getDbUtil()->encodePlaceHolders(@values);
  my $numRowsAffected = eval {
...
sub sql {
  ...
  if ($sql =~ m{ (select[ ].*?) \s limit \s+ ([\s\d,]+) \z }xmsis ) {
    ...
    $obj->_debug( 'sql(' . $obj->_expandPH($sql, @placeholders) . ')', ($sql =~ m{ \A \s* select }xmsi ? 2 : ()) );
...
sub limitSelect {
  ...
  if ($rowCount > 0) {
    ...
    $obj->_debug("Using manual limit($start,$rowCount)", 2);
  }
  ...
  $obj->_debug('limitSelect: ' . $obj->_expandPH($sql, @placeholders), 2);
...
sub fetch {
  ...
  $obj->_debug('fetch(' . $obj->_expandPH($sql, @placeholders) . ')', 2);
...
sub fetchHashRef {
  ...
  $obj->_debug('fetchHashRef(' . $obj->_expandPH($sql, @placeHolders) . ')', 2);
  my @encodedPlaceHolders = $obj->_getDbUtil()->encodePlaceHolders(@placeHolders);
  my $result = $obj->getDbh()->selectrow_hashref($sql, undef, @encodedPlaceHolders);
  $result    = $obj->_getDbUtil()->decodeResult($result);
  $obj->cacheSQL( $sql, \@placeHolders, [$result] || [] );
...
sub sqlHashRef {
  ...
  $obj->_debug('sqlHashRef(' . $obj->_expandPH($sql, @placeholders) . ')', 2);
  my @encodedPlaceHolders = $obj->_getDbUtil()->encodePlaceHolders(@placeholders);
...
sub do {
  ...
  $obj->_debug('do(' . $obj->_expandPH($sql, @placeholders) . ')');
  my @encodedPlaceHolders = $obj->_getDbUtil()->encodePlaceHolders(@placeholders);
...
sub getCachedSql {
  ...
  if ( $obj->{o2DBCacheHandler}->sqlIsCachable($sql) ) {
    ...
    $obj->_debug('"' . $obj->_expandPH( $sql, @{$placeholders} ) . '" was cached, retVal:' . $cacheVal, 2) if $cacheVal;
    return $cacheVal;
  }
